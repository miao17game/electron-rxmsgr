...
export function createServiceMessenger<T extends IContext, S extends Object, C extends Object>(msgs: T) {
  ...
  return (options: WrappedServiceMessenger<T, S, C>): IServiceMessageTarget<T> => {
    // for send
    ...
    ipcMain.on(MessengerToken.ClientEvent, (event, { key, data, token }) => {
      ...
      handler({ data, context: context["host"], token, send: invoker.bind(this, event.sender) });
    });
    // for invoke
    for (const [key, handler] of Object.entries(options)) {
      ...
      ipcMain.handle(HANDLER_KEY, async (event, { data, token }) => {
        try {
          return await handler({ data, context: context["host"], token, send: invoker.bind(this, event.sender) });
        } catch (error) {
          return {
            __error: true,
